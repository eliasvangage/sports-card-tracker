      ...
      <div className={`border-b border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,0.06),rgba(255,255,255,0.015))] ${compact ? "p-3" : "p-4"}`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-full truncate text-[11px] font-bold text-slate-400" title={data?.query}>
  ...
  return (
    <div className="grid gap-3">
      <div className={`rounded-xl border border-white/10 bg-[#0d111a] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] ${compact ? "p-3" : "p-4"}`}>
        <div className={compact ? "grid gap-3" : "grid gap-3 md:grid-cols-[minmax(0,1fr)_260px]"}>
          <div className="min-w-0">
            ...
            <p className="mt-2 text-[11px] font-bold leading-4 text-slate-400">
              Exact active listings filtered by card identity.
            </p>
          </div>
          <div className={`${compact ? "grid grid-cols-2 gap-2 text-left" : "grid grid-cols-2 gap-2"}`}>
            <div className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
              <p className="text-xl font-black text-white">{data.samples}</p>
              <p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">
                listing{data.samples === 1 ? "" : "s"}
              </p>
            </div>
            <div className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
              <p className="text-sm font-black text-white">{formatMoney(data.lowPrice)}</p>
              <p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">low</p>
              <p className="mt-1 text-sm font-black text-white">{formatMoney(data.highPrice)}</p>
              <p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">high</p>
            </div>
          </div>
        </div>
      </div>
